...
export default function PaymentsPage() {
 ...
 const [recordedPayments, setRecordedPayments] = useState<PaymentRow[]>([]);
 ...
 useEffect(() => {
 setRecordedPayments(visiblePayments);
 }, [visiblePayments]);

 const bankRecords: BankRecord[] = [
 { id: "bank-rec-001", reference_number: "MM-TIG-123456789", amount: 793334, payment_method: "mobile_money", source: "mobile_settlement" },
 { id: "bank-rec-002", reference_number: "MM-MPESA-987654321", amount: 793334, payment_method: "mobile_money", source: "mobile_settlement" },
 { id: "bank-rec-003", reference_number: "NMB-TXN-2024-001", amount: 823333, payment_method: "bank_transfer", source: "bank_statement" },
 { id: "bank-rec-004", reference_number: "CASH-2024-001", amount: 510000, payment_method: "cash", source: "cash_sheet" },
 // unmatched incoming settlement to test reconciliation queue
 { id: "bank-rec-005", reference_number: "MM-AIRTEL-NEW-404", amount: 650000, payment_method: "mobile_money", source: "mobile_settlement" },
 ];

 const getReconciliationStatus = (
 payment: PaymentRow
 ): { status: ReconciliationStatus; variance: number; note: string } => {
 const bankMatch = bankRecords.find((record) => record.reference_number === payment.reference_number);

 if (payment.notes?.includes("[MANUAL_COLLECTION]") && !bankMatch) {
 return {
 status: "manual_review",
 variance: 0,
 note: "Manual collection captured by loan officer; waiting reconciliation against bank/account records.",
 };
 }
 ...
 note: "Recorded payment has no matching bank/account reference.",
 };
 }
 ...
 status: "matched",
 variance: 0,
 note: "Recorded payment matches bank/account record.",
 };
 };
 ...
 { matched: 0, underpaid: 0, overpaid: 0, manual_review: 0, unmatched: 0 } as Record<ReconciliationStatus, number>
 );
 ...
 const selectedLoanData = visibleLoans.find((loan) => loan.id === selectedLoan);
 ...
 customer_id: selectedLoanData.customer_id,
 amount,
 payment_method: paymentMethod,
          ...
          <Card>
            ...
            <CardContent className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
              ...
              <div className="rounded-lg border border-border p-3">
                ...
                <p className="text-xl font-semibold text-destructive">{reconciliationSummary.underpaid}</p>
              </div>
              <div className="rounded-lg border border-border p-3">
              ...
              <div className="rounded-lg border border-border p-3">
                ...
                <p className="text-xl font-semibold text-destructive">{reconciliationSummary.unmatched}</p>
              </div>
            </CardContent>
          </Card>
 ...
 <Button onClick={handleManualReconciliation} disabled={!selectedPaymentId || !selectedBankRecordId}>
 ...
 <p className="max-w-56 text-xs text-muted-foreground">{reconciliation.note}</p>
 </div>
 </TableCell>
