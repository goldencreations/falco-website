...
export function GeneralLedgerWorkspace() {
  ...
  async function submitSearch(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await requestJson<{ data: SearchResult[] }>(`loan-ledgers?search=${encodeURIComponent(term)}`);
      setResults(response.data); setSearched(true);
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Unable to search loan ledgers."); }
  ...
  return <div className="flex min-h-0 flex-1 flex-col overflow-auto bg-muted/25">
    <header className="sticky top-0 z-20 flex min-h-16 items-center justify-between gap-3 border-b bg-background/95 px-4 py-3 backdrop-blur md:px-6">
      <div className="flex items-center gap-3"><SidebarTrigger /><div><h1 className="text-base font-semibold">Customer Loan Ledger</h1><p className="text-xs text-muted-foreground">Search one customer or loan and confirm its debit and credit entries.</p></div></div>
    ...
    <main className="mx-auto w-full max-w-7xl space-y-5 p-4 md:p-6">
      ...
      {selected ? <LoanLedgerDetail ledger={selected} onBack={() => setSelected(null)} /> : <SearchResults results={results} searched={searched} loading={loading} onOpen={openLedger} />}
    </main>
  </div>;
}

function SearchResults({ results, searched, loading, onOpen }: { results: SearchResult[]; searched: boolean; loading: boolean; onOpen: (loanId: number) => void }) {
  if (!searched && !loading) return <div className="rounded-xl border border-dashed bg-background px-6 py-16 text-center"><Search className="mx-auto mb-3 size-8 text-muted-foreground" /><p className="font-medium">Search to open a customer&apos;s loan ledger</p><p className="mt-1 text-sm text-muted-foreground">Each loan is shown separately so its debits and credits can be checked clearly.</p></div>;
  ...
  if (!results.length) return <div className="rounded-xl border bg-background px-6 py-12 text-center text-sm text-muted-foreground">No customer loans matched that search.</div>;

  return <Card><CardHeader><CardTitle className="text-base">Matching loans ({results.length})</CardTitle></CardHeader><CardContent className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Customer</TableHead><TableHead>Loan / application</TableHead><TableHead className="text-right">Debit</TableHead><TableHead className="text-right">Credit</TableHead><TableHead>Balance check</TableHead><TableHead /></TableRow></TableHeader><TableBody>{results.map((result) => <TableRow key={result.id}>
