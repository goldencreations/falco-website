...
export default function OfficerDashboardPage() {
 ...
 const [appCounts, setAppCounts] = useState({ pending: 0, approved: 0, total: 0 });
 const [dataLimited, setDataLimited] = useState(false);

 const branchId = user?.branch_id?.trim() ?? "";

 useEffect(() => {
 if (!user || user.role !== "loan_officer" || !branchId) return;
 ...
 void (async () => {
 const [metricsRes, customersRes, appsRes] = await Promise.all([
 fetch(`/api/falco/dashboard/metrics?${params.toString()}`),
 fetch(`/api/customers/my-customers?${params.toString()}&page_size=200`),
 fetch(`/api/applications?${params.toString()}&page_size=200`),
 ]);

 if (cancelled) return;

 const limited =
 !metricsRes.ok || !customersRes.ok || !appsRes.ok;
 setDataLimited(limited);

 const metricsJson = (await readJsonSafe(metricsRes)) as MetricsPayload | null;
 if (metricsJson) setMetrics(metricsJson);

 const customersJson = await readJsonSafe(customersRes);
 if (customersJson && typeof customersJson === "object" && "customers" in customersJson) {
 setCustomerCount(
 Array.isArray((customersJson as { customers: unknown }).customers)
 ? (customersJson as { customers: unknown[] }).customers.length
 : extractCustomersList(customersJson).length
 );
 } else if (customersJson) {
 setCustomerCount(extractCustomersList(customersJson).length);
 }

 const appsJson = await readJsonSafe(appsRes);
 if (appsJson) {
 const apps = extractApplicationsList(appsJson).filter(
 ...
 () =>
 branchId ? `${t("common.branch")} ${branchId}` : t("common.branch"),
 [branchId, t]
 );
 ...
 Some dashboard figures could not be loaded from the server. Counts below may be
 incomplete; try refreshing or contact support if this continues.
 </p>
 ) : null}

 {!branchId ? (
 <p className="rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-950">
 Assign a branch to your account to load portfolio metrics and customer lists.
 </p>
 ) : null}
 ...
 <CardDescription>{t("officer.applicationsOriginated")}</CardDescription>
 </CardHeader>
 ...
 <CardDescription>{t("officer.quickActionsDesc")}</CardDescription>
 </CardHeader>
 <CardContent className="space-y-3 text-sm text-muted-foreground">
 <p>{t("officer.quickActionsBody")}</p>
 <div className="flex items-center gap-2 text-foreground">
 <TrendingUp className="h-4 w-4 text-emerald-600" />
 <span className="text-sm font-medium">{t("officer.recoveryMetrics")}</span>
 </div>
 </CardContent>
 </Card>
 </section>
 ...
 <p className="mt-1 text-xl font-semibold">{appCounts.pending}</p>
 </div>
 <div className="rounded-lg border p-3">
