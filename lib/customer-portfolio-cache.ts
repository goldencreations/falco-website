...
export function getCachedCustomerPortfolio(id: string): CustomerPortfolioData | null {
  const now = Date.now();
...
export function setCachedCustomerPortfolio(id: string, data: CustomerPortfolioData) {
  const entry: CacheEntry = { data, expiresAt: Date.now() + CACHE_MS };
  memory.set(id, entry);
  writeSession(id, entry);
}

export function invalidateCustomerPortfolioCache(id?: string) {
  if (id) {
    memory.delete(id);
    ...
    invalidateFetchCache(`/api/customers/${id}/portfolio`);
    return;
  }
  memory.clear();
  if (typeof window !== "undefined") {
